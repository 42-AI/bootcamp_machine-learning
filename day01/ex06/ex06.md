...
where $X$ is the training dataset matrix, $\theta_{NE}$ the parameter's hypothesis vector obtained with the normal equation, $Y$ are the output vector.
...
You are expected to:
* Add the method **normalequation** in the class **__MyLinearRegression__** as per the instructions given below:
```python
...
* Calculate the MSE of the data and the predicted data generate with the model train with the linear gradient descent, then with normal equation technique.
