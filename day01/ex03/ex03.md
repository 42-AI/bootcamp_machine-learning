...
\nabla(J)_1 = \frac{1}{m}\sum_{i=1}^{m}(h_{\theta}(x^{(i)}) - y^{(i)})x_{i}
$$
...
- $h_{\theta}(x^{(i)})$ corresponds to our prediction for $y^{(i)}$: the result of the dot product of the vector $\theta$ and the vector $x^{(i)}$
