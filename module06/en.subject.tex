...
Where:
\begin{itemize}
  \item $\nabla(J)$ is the gradient vector of size $2$, (this strange symbol : $\nabla$ is called nabla),
...
Where:
\begin{itemize}
  \item $\nabla(J)$ is a vector of dimension $2$,
...
# Example 0.1:
lr1.loss_elem_(lr1.predict(x),y)
# Output:
array([[710.45867381],
       [364.68645485],
       [469.96221651],
...
# Example 1.2:
MyLR.loss_elem_(lr2.predict(x),y)
# Output:
array([[486.66604863],
       [115.88278416],
       [ 84.16711596],
...
$$
\begin{matrix}
 x'^{(i)} = \frac{x^{(i)} - \frac{1}{m} \sum_{i = 1}^{m} x^{(i)}}{\sqrt{\frac{1}{m} \sum_{i = 1}^{m} (x^{(i)} - \frac{1}{m} \sum_{i = 1}^{m} x^{(i)})^{2}}} & &\text{ for $i$ in $1, ..., m$} 
\end{matrix}
$$
