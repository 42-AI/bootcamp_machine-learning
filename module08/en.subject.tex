...
\begin{itemize}
  ...
  \item $X'$ is a matrix of dimension $(m \times (n + 1))$, the design matrix onto which a column of $1$'s is added as a first column,
...
\section*{Objective}
% --------------------------------- %
Understanding and manipulation loss function concept in case of logistic regression.\
You must implement the following formula as a function:  
...
\begin{itemize}
  \item $\hat{y}$ is a vector of dimension $m$, the vector of predicted values,
  \item $\hat{y}^{(i)}$ is the i$^\text{th}$ component of the $\hat{y}$ vector,
...
\section*{Objective}
% --------------------------------- %
Understand and manipulate concept of gradient in the case of logistic formulation.\
You must implement the following formula as a function:  
...
\section*{Instructions}
% --------------------------------- %
n the \texttt{vec\_log\_gradient.py} file, write the following function as per the instructions below:
...
\section*{Objective}
% --------------------------------- %
The time to use everything you built so far has come!
Demonstrate your knowledge by implementing a logistic regression classifier using the gradient descent algorithm.
...
\begin{itemize}
  ...
  \item \texttt{loss\_elem\_(self, x)},
  \item \texttt{loss\_(self, x, y)},
  \item \texttt{fit\_(self, x, y)},
\end{itemize}

You have already written these functions, you will just need a few adjustments so that they all work well within your \texttt{MyLogisticRegression} class.
Obviously, you should handles wrong type and incoherent values for parameters.
...
You are expected to write 2 programs that will use Logistic Regression to predict from which planet each citizen comes from, based on the other variables found in the census dataset.

But wait... what?
There are four different planets!
How do you make a classifier discriminate between 4 categories?
...
\begin{enumerate}
  ...
  \item Based on the selected Space Zipcode ($0$, $1$, $2$, $3$), generate a new \texttt{numpy.array} to label each citizen according to your new selection criterion:
  \begin{itemize}
