...
* `size` : size of the vector &rarr; `Vector([0.0, 1.0, 2.0, 3.0]).size == 4`

You should be able to initialize the object with:
* a list of floats: `Vector([0.0, 1.0, 2.0, 3.0])`  
* a size: `Vector(3)` &rarr; the vector will have `values = [0.0, 1.0, 2.0]`
* a range: `Vector((10,15))` &rarr; the vector will have `values = [10.0, 11.0, 12.0, 13.0, 14.0]`
