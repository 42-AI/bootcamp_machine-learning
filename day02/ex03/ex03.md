...
- $X'$ is a matrix of dimension $m * (n + 1)$, the matrix of examples onto which a column of ones is added as a first column
...
- $x_j^{(i)}$ is the intersection of the $i^{th}$ row and the $j^{th}$ column of the $X$ matrix
